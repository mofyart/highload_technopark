...
> **Примечание.** Формула: `RPS_регион = RPS_всего × доля_аудитории`, аналогично для трафика. «Остальной мир» (§2.1.1) не разбит по странам в наших данных; отнесён к secondary-региону (Казахстан) как консервативное допущение для ёмкостного планирования — фактическую маршрутизацию каждого конкретного клиента всё равно определяет latency-based DNS (§3.4) по измеренному RTT, а не эта таблица.
