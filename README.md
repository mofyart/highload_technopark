...
> **Примечание о СПб-площадке.** Помимо 4 зон VK Cloud, у VK подтверждён отдельный действующий ЦОД в Ленинградской обл. (ИЦВА, Всеволожский р-н) и строящийся (Корабельная ул., СПб) — оба под управлением дочерней структуры «М100». RTT Москва↔СПб ≈ 10–15 мс — на порядок меньше, чем между настоящими GSLB-регионами, поэтому это не отдельный регион для latency-based DNS (§3.4), а вторая физическая площадка primary-региона: защищает от аварии/отключения одного города (disaster recovery), а не снижает latency для пользователей.
...
| GSLB | Primary — DR-площадка | ИЦВА (действ.) / Корабельная ул. (строится) | Санкт-Петербург / Ленобласть |
