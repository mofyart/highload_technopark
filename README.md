...
> **Источник:** «Ведомости», 16.03.2015 («Mail.ru Group ввела плату за хранение данных в облаке»), данные на март 2015, приведено по [Википедии](https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D0%BB%D0%B0%D0%BA%D0%BE_Mail). Цифры — доля **зарегистрированных аккаунтов** (~24 млн «облаков»), а не MAU. Пересчёт «после 2022» — авторская экстраполяция, в первоисточнике отсутствует.

#### 2.1.2 Средний размер хранилища на пользователя (по типам данных)

| Тип данных | Объём на пользователя, ГБ | Число файлов на пользователя, шт. |
|---|---|---|
| Фотографии |  |  |
| Видео |  |  |
| Документы и прочее |  |  |
...
> `объём/пользователя = объём(тип) / MAU`; `файлов/пользователя = файлы(тип) / MAU`. Исходные — глава 1 (335 ПБ, 45,5 млрд файлов; 28 млрд фото = 70 ПБ; 2,5 млрд видео).
