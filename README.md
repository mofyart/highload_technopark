...
|---|---|---|---|
| Auth | Авторизация, refresh-токен | Короткие запросы, read-heavy | **Eventual** — проверка пароля и подписи JWT stateless и читает редко меняющееся хранилище; согласованность нужна лишь отзыву refresh-токенов и антибрутфорс-счётчикам |
...
> **Вывод.** Строгая консистентность нужна только домену **Quota** — это счётчик, при параллельном инкременте в двух ДЦ он разъедется и квоту можно превысить, поэтому его держим в primary-регионе. **Auth** в stateless-JWT-схеме распределяется по всем ДЦ (проверка подписи локально), но редко меняющееся общее состояние — отзыв refresh-токенов и антибрутфорс-счётчики — реплицируется из primary. **Object Storage** требует sticky-маршрутизации на весь сеанс загрузки/скачивания. **Public/Edge** — единственный домен, для которого anycast безопасен (см. §3.5).
