...
Глобальная балансировка (**GSLB — Global Scale Load Balancing**) отвечает на вопрос «в какой из нескольких географически разнесённых ДЦ направить запрос пользователя», в отличие от локальной балансировки (**LSLB**, глава 9) — «на какой из бекендов внутри одного ДЦ». Материал главы построен по методологии лекции «Проектирование высоконагруженных систем. Лекция 3. Глобальная балансировка нагрузки» (Быков А., VK, 2023).
...
| Домен | Функции MVP (§1.3) | Характер трафика | Требование к ДЦ |
|---|---|---|---|
| Auth | Авторизация, refresh-токен | Короткие запросы, control-plane | Строгая консистентность — единый источник истины (primary-регион) |
| Metadata/FS | Листинг, CRUD папок | Короткие запросы, читаются часто | Консистентность на запись, реплики на чтение из вторичных ДЦ |
| Quota | Проверка/обновление квоты | Короткие запросы, control-plane | Строгая консистентность — единый источник истины (primary-регион) |
| Object Storage (data-plane) | Многопоточная загрузка, скачивание | Длинные multipart-сессии (§2.2.3: N=1,6 блока/файл), основной объём трафика (§2.2.2: ingress+egress файлов ≈ 50 Гбит/с из 53,7) | Привязка к одному ДЦ на всю сессию — переключение ДЦ рвёт TCP-соединение |
| Public/Edge | Публичный доступ по ссылке, превью | Короткие кэшируемые GET, читаются анонимно | Не требует консистентности — кандидат на edge/anycast-кеш |

> **Вывод.** Auth и Quota нельзя размазывать active-active между регионами без распределённого консенсуса — оставляем их в primary-регионе с редким кросс-региональным вызовом. Object Storage требует sticky-маршрутизации на весь сеанс загрузки/скачивания. Public/Edge — единственный домен, для которого anycast безопасен (см. §3.5).
...
> **Примечание.** Формула: `RPS(регион) = RPS_итого × доля_аудитории`, аналогично для трафика. Домены Auth/Quota (§3.1) физически не распределяются по этим долям — они всегда идут в primary-регион независимо от места запроса; таблица отражает распределение **суммарного** запроса на границе GSLB, до маршрутизации отдельных доменов.

### 3.4 Схема DNS-балансировки

Двухуровневая схема:

1. **Между регионами (GSLB) — Latency-based DNS** (по образцу AWS Route 53, слайд презентации): резолвер получает адрес ДЦ с минимальным RTT из региона запроса. Выбран вместо Geo-based DNS (по IP-геолокации), т.к. latency-based устойчивее к неточной геолокации резолвера провайдера и лучше отражает реальную сетевую доступность. Веса ответа пропорциональны распределению из §3.3 (74% / 4,5% / 21,5% → RU).
...
- **Единый источник истины для Auth/Quota** в primary-регионе (RU) с асинхронной репликой в KZ и возможностью promote при аварии primary — эти домены не балансируются active-active между регионами (§3.1), в отличие от data-plane и edge-слоя.

### 3.7 Источники

- Презентация «Проектирование высоконагруженных систем. Лекция 3. Глобальная балансировка нагрузки» (Быков А., VK, 2023) — методология (GSLB/LSLB, алгоритмы DNS-балансировки, BGP Anycast).
...
- Глава 1, §1.2 — географическое распределение аудитории (источник — Википедия).
