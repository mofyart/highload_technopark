...
> **Примечание.** Формулы: `ГБ/сутки = Action_per_day * avg_size / 82; `Гбит/с_peak = RPS_peak × avg_size × 8`, где `RPS_peak = RPS_avg × K_peak`. Допущения: `avg_size = 7,4 МБ` (из §2.2.1); `K_peak = 4` (пиковый час ≈ 1/6 суток); превью = 4 листинга/день × ~30 миниатюр × ~20 КБ. Авторизация, квота и метаданные по трафику пренебрежимо малы — не учитываются.
