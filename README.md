...
|---|---|---|---|
| Auth | Авторизация, refresh-токен | Короткие запросы, read-heavy | Слабая: проверка пароля и подписи JWT — stateless, читается редко меняющееся хранилище; согласованность нужна только отзыву refresh-токенов и антибрутфорс-счётчикам |
| Metadata/FS | Листинг, CRUD папок | Короткие запросы, читаются часто | Чтение — eventual; запись — через sticky (один писатель на пользователя) |
| Quota | Проверка/обновление квоты | Короткие запросы, control-plane | **Строгая**: счётчик занятого места, единый источник истины (primary-регион) |
| Object Storage (data-plane) | Многопоточная загрузка, скачивание | Длинные multipart-сессии (§2.2.3: N=1,6 блока/файл), основной объём трафика (§2.2.2: ingress+egress файлов ≈ 50 Гбит/с из 53,7) | Слабая на чтение (blob неизменяем после записи) + sticky на сессию — переключение ДЦ рвёт TCP-соединение |
| Public/Edge | Публичный доступ по ссылке, превью | Короткие кэшируемые GET, читаются анонимно | Не требует — eventual-кэш, кандидат на edge/anycast-кеш |
