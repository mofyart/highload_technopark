...
|---|---|---|---|
| **Primary — Россия (Москва, DR-пара с СПб)** | ≈ 74% | 4 зоны доступности Tier III (MS1, GZ1, ME1, PA2) + резервная площадка Ленобласть/СПб | Основная масса DAU; требование 152-ФЗ о локализации персональных данных граждан РФ |
| **Secondary — Казахстан (Косшы / Астана)** | ≈ 4,5% (+ 12,5% остальной мир, ёмкостное допущение §3.3) | 2 зоны доступности Tier III (QazCloud + KazTeleport) | Локальное законодательство Казахстана (Закон РК №94-V «О персональных данных и их защите», ст. 12 п. 2 — обязательная локализация БД на территории РК); снижает RTT для Центральной Азии |
| **Без выделенного ДЦ — Турция** | ≈ 9% | — обслуживаются из primary | Аудитория одной страны, но без юридического требования локального размещения (KVKK регулирует трансграничную передачу, не обязывает хранить данные частных B2C-сервисов в стране); сегмент не достигает порога, оправдывающего собственный ДЦ; обслуживается через международный транзит primary-региона (upstream AS47764 включает Telia AS1299); кэшируемая часть трафика (превью, публичные ссылки) выносится на edge/CDN-узлы ближе к пользователю |
...
| Казахстан (secondary) + остальной мир | 4,5% + 12,5% = 17% | 206 / 821 | 9,1 / 24 630 |
| Турция (обслуживаются из primary) | 9% | 109 / 435 | 4,8 / 13 040 |
...
Конкретные ЦОД, участвующие в схеме:

| Уровень балансировки | Регион / зона | ЦОД | Город |
|---|---|---|---|
| GSLB | Primary — Россия | (см. зоны ниже) | Москва |
| GSLB | Primary — DR-площадка | ИЦВА (действ.) Корабельная ул.| Санкт-Петербург / Ленобласть |
| GSLB | Secondary — Казахстан | QazCloud (QAZ) / KazTeleport (KTP) | Косшы / Астана |
| LSLB (Weighted RR) | Зона GZ1 | Гознак | Москва |
| LSLB (Weighted RR) | Зона MS1 | DataLine NORD4 | Москва |
| LSLB (Weighted RR) | Зона ME1 | Ростелеком-ЦОД «Медведково» | Москва |
| LSLB (Weighted RR) | Зона PA2 | «Пахра» | Домодедово (Моск. обл.) |
...
- **Внутрирегиональное георезервирование (Москва ↔ СПб)** — primary-регион размещён на двух физически разнесённых площадках (§3.2); переключение между ними — задача LSLB/репликации внутри региона (глава 9), а не latency-based DNS (§3.4), т.к. это защита от аварии города, а не выбор ближайшего к пользователю ДЦ.
...
- [cloud.vk.ru — курс «Архитектура VK Cloud»](https://cloud.vk.ru/cloud-native-courses/advanced/arhitektura-vk-cloud/) — адреса ЦОД зон GZ1/MS1/ME1/PA2.
- [vprigorode.ru — ЦОД ИЦВА, Новосергиевка](https://www.vprigorode.ru/news/2025-05-30/tekhnologii_dlya_novosergyevki/), [kommersant.ru — покупка цеха ТЭЦ-14 под ЦОД в СПб](https://www.kommersant.ru/doc/7156395) — СПб-площадка primary-региона.
- [mforum.ru/analit/tsod.htm](http://www.mforum.ru/analit/tsod.htm) — переход VK на собственные ЦОД (2023).
- [online.zakon.kz — Закон РК №94-V «О персональных данных и их защите»](https://online.zakon.kz/Document/?doc_id=39801023) — требование локализации персональных данных в Казахстане (ст. 12 п. 2); [gun.av.tr — Data Residency in Turkey](https://gun.av.tr/insights/guides/data-residency-in-turkey-for-2025) — KVKK не требует локализации для частных B2C-сервисов вне регулируемых секторов.
- [forbes.kz](https://forbes.kz/articles/kazahstan-stroit-tsifrovoy-shelkovyy-put-podklyuchayas-kevrope-vobhod-rossii-c280b8) - Транскаспийская ВОЛС / Digital Silk Way — СП «Казахтелекома» и AzerTelecom (CaspiNet), магистраль DWDM 48×100 Гбит/с, Казахстан позиционируется как транзитный узел Европа–Азия .
