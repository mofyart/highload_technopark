...
| Видео | — | 87 | https://habr.com/ru/news/887932/ — 2,5 млрд видео (объём не раскрыт) |
| Документы и прочее | — | 524 | вычислено: 45,5 − 28 − 2,5 = 15 млрд (из https://habr.com/ru/news/887932/) |
| **Итого** | 11,7 | 1591 | https://habr.com/ru/news/887932/ — 335 ПБ, 45,5 млрд файлов |

> Расчёт на **глобальный MAU ≈ 28,6 млн** (= 21,2 млн Россия / 0,74, по доле России ~74% из §2.1.1): `объём/пользователя = объём(тип) / MAU`, `файлов/пользователя = файлы(тип) / MAU` (значения округлены до целых). Это **верхняя оценка на активного пользователя** — среднее по всем аккаунтам ниже, но VK не раскрывает число аккаунтов (ни для Облака, ни для Почты). Объём видео и документов в первоисточнике не раскрыт — оставшиеся 265 ПБ (335 − 70) требуют дооценки.

#### 2.1.3 Среднее количество действий пользователя в день (по типам запросов)

Типы запросов — по функционалу MVP (§1.3).

| Тип действия (функция MVP) | Действий в день на пользователя |
|---|---|
| Авторизация (вход / refresh-токен) |  |
| Загрузка файлов (многопоточная) |  |
| Скачивание файлов |  |
| Операции файловой системы (создание / переименование / перемещение / удаление / листинг) |  |
| Публичный доступ по ссылке (создание / просмотр / скачивание) |  |
| Проверка / обновление квоты (оверквота) |  |
