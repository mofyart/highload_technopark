...
|---|---|---|---|
| Auth | Авторизация, refresh-токен | Короткие запросы, read-heavy | **Stateless для проверки, строгая для security-записей** — проверка JWT самодостаточна (общего состояния нет, все ДЦ вернут одинаково); но отзыв refresh-токена, смена пароля и бан должны мгновенно разойтись по всем ДЦ через primary, иначе украденный токен продолжит работать |
| Metadata/FS | Листинг, CRUD папок | Короткие запросы, читаются часто | **Eventual** — листинг допускает краткое отставание реплики; редкие CRUD-записи идут к одному писателю на пользователя (sticky), поэтому конфликтов нет |
...
| Object Storage (data-plane) | Многопоточная загрузка, скачивание | Длинные multipart-сессии (§2.2.3: N=1,6 блока/файл), основной объём трафика (§2.2.2: ingress+egress файлов ≈ 50 Гбит/с из 53,7) | **Eventual** — blob после записи неизменяем, реплики сходятся без конфликта; multipart-сессию фиксируем за одним ДЦ (sticky), иначе переключение рвёт TCP |
...
> **Вывод.** Строгая консистентность нужна **Quota** (счётчик — при независимом инкременте в двух ДЦ разъедется и квоту можно превысить) и **security-записям Auth** (отзыв refresh-токена, смена пароля, бан — должны мгновенно действовать на всех ДЦ, иначе украденный токен продолжит работать). Обе держим в primary-регионе. Сама проверка Auth (JWT) — stateless, идёт локально в каждом ДЦ. **Object Storage** требует sticky-маршрутизации на весь сеанс загрузки/скачивания. **Public/Edge** — единственный домен, для которого anycast безопасен (см. §3.5).

### 3.2 Обоснование расположения ДЦ (влияние на продуктовые метрики)

Расположение ДЦ определяется географией аудитории (§2.1.1, примечание о географии): Россия ≈ 74%, Турция ≈ 9%, Казахстан ≈ 4,5%, остальной мир ≈ 12,5%.

| Регион | Доля аудитории | ДЦ | Обоснование |
|---|---|---|---|
| **Primary — Россия (Москва)** | ≈ 74% | 3 зоны доступности Tier III | Основная масса DAU; требование 152-ФЗ о локализации персональных данных граждан РФ; исторически основная инфраструктура VK (аренда DataPro Авиамоторная → с 2023 собственные ЦОД, до 3–4 новых площадок, ≈ 40 млрд ₽ инвестиций) |
| **Secondary — Казахстан (Косшы / Астана)** | ≈ 4,5% | 2 зоны доступности Tier III (QazCloud + KazTeleport) | Локальное законодательство Казахстана (Постановление Правительства РК №832 о резервировании); снижает RTT для Центральной Азии; уже существует как реальная инфраструктура VK Cloud — переиспользуем модель, а не проектируем с нуля |
| **Без выделенного ДЦ — Турция + остальной мир** | ≈ 21,5% | — (обслуживаются из primary) | Аудитория фрагментирована по множеству стран, ни один сегмент не достигает порога, оправдывающего собственный ДЦ; обслуживается через международный транзит primary-региона (upstream AS47764 включает Telia AS1299); кэшируемая часть трафика (превью, публичные ссылки) выносится на edge/CDN-узлы ближе к пользователю (см. §3.5) без переноса самого хранилища |

> **Примечание.** Данные о реальной инфраструктуре VK использованы как референс порядка величин и обоснование архитектурного решения (2 региона вместо globally distributed anycast-сети): [bgp.tools/as/47764](https://bgp.tools/as/47764) — AS47764, 13 upstream-провайдеров, вкл. международный (Telia); [vk.company/ru/press/releases/11468](https://vk.company/ru/press/releases/11468/) — запуск региона в Казахстане (QazCloud); [vkcloud.kz/blog](https://vkcloud.kz/blog/vk-tech-zapustila-vtoruyu-zonu-dostupnosti-vk-cloud-v-respublike-kazakhstan/) — вторая зона доступности (KazTeleport, Астана); [anti-malware.ru](https://www.anti-malware.ru/news/2026-05-12-111332/50002) — 3 зоны доступности Tier III в Москве + собственный ЦОД; [mforum.ru](http://www.mforum.ru/analit/tsod.htm) — переход VK с арендованных мощностей на строительство собственных ЦОД (2023).
...
> **Примечание.** Формула: `RPS(регион) = RPS_итого × доля_аудитории`, аналогично для трафика. Домен **Quota** (§3.1) физически не распределяется по этим долям — всегда идёт в primary-регион независимо от места запроса; таблица отражает распределение **суммарного** запроса на границе GSLB, до маршрутизации отдельных доменов.
...
2. **Внутри региона (между 3 зонами доступности Москвы) — Weighted Round-Robin DNS** с весами, равными доле ёмкости каждой зоны, либо делегирование на L4/L7 (глава 9, LSLB) — задержка между зонами одного региона мала, поэтому нет смысла терять время на DNS TTL ради выбора конкретной зоны.
...
> **Примечание.** Anycast для собственной AS (по образцу AS47764) требует отдельной сети /24 и BGP-анонса из каждой точки присутствия (недостаток слайда — «необходима своя AS и сеть /24»). Для сервиса с 2 регионами (§3.2) это оправдано только для узкого edge-слоя, а не для всего трафика — разворачивать anycast-сеть уровня Cloudflare/Google под 21,5% некэшируемого трафика (Турция + остальной мир, §3.3) экономически не оправдано.
...
- **Георезервирование объектного хранилища** между RU и KZ по умолчанию (переиспользуем реальную практику VK Cloud, §3.2) — при отказе одного региона данные остаются доступны из реплики другого без остановки сервиса.
- **Единый источник истины для Quota** в primary-регионе (RU) с асинхронной репликой в KZ и возможностью promote при аварии primary; для **Auth** реплицируется только общее состояние (отзыв токенов, антибрутфорс), а сама проверка идёт локально в каждом ДЦ. Quota не балансируется active-active между регионами (§3.1), в отличие от data-plane и edge-слоя.
